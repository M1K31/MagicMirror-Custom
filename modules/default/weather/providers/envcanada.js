...
WeatherProvider.register("envcanada", {
   ...
   */
     ...
     */
		if (
			ECdoc.querySelector("siteData currentConditions temperature").textContent
		) {
			currentWeather.temperature = ECdoc.querySelector(
				"siteData currentConditions temperature"
			).textContent;
		...
		currentWeather.windSpeed = WeatherUtils.convertWindToMs(
			ECdoc.querySelector("siteData currentConditions wind speed").textContent
		);

		currentWeather.windFromDirection = ECdoc.querySelector(
			"siteData currentConditions wind bearing"
		).textContent;

		currentWeather.humidity = ECdoc.querySelector(
			"siteData currentConditions relativeHumidity"
		).textContent;
     ...
     */
		if (this.config.showFeelsLike) {
			...
			if (ECdoc.querySelector("siteData currentConditions windChill")) {
				currentWeather.feelsLikeTemp = ECdoc.querySelector(
					"siteData currentConditions windChill"
				).textContent;
			}

			if (ECdoc.querySelector("siteData currentConditions humidex")) {
				currentWeather.feelsLikeTemp = ECdoc.querySelector(
					"siteData currentConditions humidex"
				).textContent;
			}
		}
		...
		currentWeather.weatherType = this.convertWeatherType(
			ECdoc.querySelector("siteData currentConditions iconCode").textContent
		);
		...
		currentWeather.sunrise = moment(
			sunList[1].querySelector("timeStamp").textContent,
			"YYYYMMDDhhmmss"
		);
		currentWeather.sunset = moment(
			sunList[3].querySelector("timeStamp").textContent,
			"YYYYMMDDhhmmss"
		);
   ...
   */
	generateWeatherObjectsFromForecast (ECdoc) {
		...
		const foreBaseDates = ECdoc.querySelectorAll(
			"siteData forecastGroup dateTime"
		);
     ...
     */
		let nextDay = 0;
		let lastDay = 0;
		const currentTemp = ECdoc.querySelector(
			"siteData currentConditions temperature"
		).textContent;
     ...
     */
		weather.weatherType = this.convertWeatherType(
			foreGroup[0].querySelector("abbreviatedForecast iconCode").textContent
		);
     ...
     */
       ...
       */
			...
			weather.weatherType = this.convertWeatherType(
				foreGroup[stepDay].querySelector("abbreviatedForecast iconCode")
					.textContent
			);
   ...
   */
	generateWeatherObjectsFromHourly (ECdoc) {
		...
		const baseHours = ECdoc.querySelectorAll(
			"siteData hourlyForecastGroup dateTime"
		);
     ...
     */
		const hourGroup = ECdoc.querySelectorAll(
			"siteData hourlyForecastGroup hourlyForecast"
		);
		...
		for (let stepHour = 0; stepHour < 24; stepHour += 1) {
			...
			const foreTime = moment(
				hourGroup[stepHour].getAttribute("dateTimeUTC"),
				"YYYYMMDDhhmmss"
			);
			...
			weather.temperature
        = hourGroup[stepHour].querySelector("temperature").textContent;

			// Capture Likelihood of Precipitation (LOP) and unit-of-measure values
			const precipLOP
        = hourGroup[stepHour].querySelector("lop").textContent * 1.0;
			...
			// Need to map EC weather icon to MM weatherType values. Always pick the first Element icon.
			weather.weatherType = this.convertWeatherType(
				hourGroup[stepHour].querySelector("iconCode").textContent
			);
   ...
   */
	setMinMaxTemps (weather, foreGroup, today, fullDay, currentTemp) {
		const todayTemp = foreGroup[today].querySelector(
			"temperatures temperature"
		).textContent;

		const todayClass = foreGroup[today]
			.querySelector("temperatures temperature")
			.getAttribute("class");
     ...
     */
		...
		const nextTemp = foreGroup[today + 1].querySelector(
			"temperatures temperature"
		).textContent;

		const nextClass = foreGroup[today + 1]
			.querySelector("temperatures temperature")
			.getAttribute("class");
   ...
   */
	setPrecipitation (weather, foreGroup, today) {
        ...
        = foreGroup[today].querySelector("precipitation accumulation amount")
					.textContent * 1.0;
			weather.precipitationUnits = foreGroup[today]
				.querySelector("precipitation accumulation amount")
				.getAttribute("units");
		}

		// Check Today element for POP
		const precipPOP
      = foreGroup[today].querySelector("abbreviatedForecast pop").textContent
        * 1.0;
		if (precipPOP > 0) {
