...
WeatherProvider.register("ukmetofficedatahub", {
   ...
   */
		...
		this.fetchWeather(this.getUrl("hourly"), this.getHeaders())
			.then((data) => {
				...
				if (
					...
					|| !data.features[0].properties.timeSeries
					|| data.features[0].properties.timeSeries.length === 0
				) {
	...
	generateWeatherObjectFromCurrentWeather (currentWeatherData) {
		...
		let forecastDataHours
      = currentWeatherData.features[0].properties.timeSeries;
		...
		for (let hour in forecastDataHours) {
			...
			if (
				nowUtc.isSameOrAfter(forecastTime)
				&& nowUtc.isBefore(moment(forecastTime.add(1, "h")))
			) {
          ...
          = forecastDataHours[hour].windDirectionFrom10m;
				currentWeather.temperature = forecastDataHours[hour].screenTemperature;
				currentWeather.minTemperature
          = forecastDataHours[hour].minScreenAirTemp;
				currentWeather.maxTemperature
          = forecastDataHours[hour].maxScreenAirTemp;
				currentWeather.weatherType = this.convertWeatherType(
					forecastDataHours[hour].significantWeatherCode
				);
				currentWeather.humidity
          = forecastDataHours[hour].screenRelativeHumidity;
          ...
          = forecastDataHours[hour].probOfPrecipitation;
				currentWeather.feelsLikeTemp
          = forecastDataHours[hour].feelsLikeTemperature;
	...
	generateWeatherObjectsFromForecast (forecasts) {
		...
		for (let day in forecastDataDays) {
          ...
          = forecastDataDays[day].nightMinScreenTemperature;
				forecastWeather.maxTemperature
          = forecastDataDays[day].dayMaxScreenTemperature;

				// Using daytime forecast values
				forecastWeather.windSpeed = forecastDataDays[day].midday10MWindSpeed;
				forecastWeather.windFromDirection
          = forecastDataDays[day].midday10MWindDirection;
				forecastWeather.weatherType = this.convertWeatherType(
					forecastDataDays[day].daySignificantWeatherCode
				);
				forecastWeather.precipitationProbability
          = forecastDataDays[day].dayProbabilityOfPrecipitation;
          ...
          = forecastDataDays[day].dayMaxScreenTemperature;
				...
				forecastWeather.feelsLikeTemp
          = forecastDataDays[day].dayMaxFeelsLikeTemp;
