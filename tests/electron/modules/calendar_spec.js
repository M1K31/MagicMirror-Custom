...
describe("Calendar module", () => {
	...
	const doTest = async (cssClass) => {
		const elem = await helpers.getElement(
			`.calendar .module-content .event${cssClass}`
		);
	...
	const doTestTableContent = async (
		table_row,
		table_column,
		content,
		row = first
	...
	describe("rrule", () => {
		it("Issue #3393 recurrence dates past rrule until date", async () => {
			await helpers.startApplication(
				...
				"07 Mar 2024 10:38:00 GMT-07:00",
				[],
				"America/Los_Angeles"
			);
	...
	describe("Exdate: LA crossover DST before midnight GMT", () => {
		it("LA crossover DST before midnight GMT should have 2 events", async () => {
			await helpers.startApplication(
				...
				"19 Oct 2023 12:30:00 GMT-07:00",
				[],
				"America/Los_Angeles"
			);
	...
	describe("Exdate: SYD crossover DST before midnight GMT", () => {
		it("LA crossover DST before midnight GMT should have 2 events", async () => {
			await helpers.startApplication(
				...
				"14 Sep 2023 12:30:00 GMT+10:00",
				[],
				"Australia/Sydney"
			);
			await expect(doTestCount()).resolves.toBe(2);
   ...
   */
	describe("sliceMultiDayEvents direct count", () => {
		it("Issue #3452 split multiday in Europe", async () => {
			await helpers.startApplication(
				...
				"01 Sept 2024 10:38:00 GMT+02:00",
				[],
				"Europe/Berlin"
			);
	...
	describe("germany timezone", () => {
		it("Issue #unknown fullday timezone East of UTC edge", async () => {
			await helpers.startApplication(
				...
				"01 Oct 2024 10:38:00 GMT+02:00",
				[],
				"Europe/Berlin"
			);
			await expect(
	...
	describe("germany all day repeating moved (recurrence and exdate)", () => {
		it("Issue #unknown fullday timezone East of UTC event moved", async () => {
			...
			await expect(
				doTestTableContent(".calendar .event", ".time", "12th.Oct")
			).resolves.toBe(true);
		});
	});
	...
	describe("chicago late in timezone", () => {
		it("Issue #unknown rrule US close to timezone edge", async () => {
			await helpers.startApplication(
				...
				"01 Sept 2024 10:38:00 GMT-5:00",
				[],
				"America/Chicago"
			);
			await expect(
				doTestTableContent(".calendar .event", ".time", "10th.Sep, 20:15")
			).resolves.toBe(true);
		});
	});
	...
	describe("berlin late in day event moved, viewed from berlin", () => {
		it("Issue #unknown rrule ETC+2 close to timezone edge", async () => {
			await helpers.startApplication(
				...
				"08 Oct 2024 12:30:00 GMT+02:00",
				[],
				"Europe/Berlin"
			);
	...
	describe("berlin whole day repeating, start moved after end", () => {
		it("some events before DST. some after", async () => {
			...
			await expect(
				doTestTableContent(".calendar .event", ".time", "30th.Oct", last)
			).resolves.toBe(true);
			await expect(
				doTestTableContent(".calendar .event", ".time", "27th.Oct", first)
			).resolves.toBe(true);
		});
	});
	...
	describe("both moved and delete events in recurring list", () => {
		it("with moved before and after original", async () => {
			await helpers.startApplication(
				...
				"08 Oct 2024 12:30:00 GMT-07:00",
				[],
				"America/Los_Angeles"
			);
	...
	describe("one event diff tz", () => {
		...
		it("viewing from further west in diff timezones", async () => {
			await helpers.startApplication(
				...
				"22 Jan 2025 14:30:00 GMT-06:00",
				[],
				"America/Chicago"
			);
			// just
	...
	describe("count and check symbols", () => {
		it("in array", async () => {
			...
			// just
			await expect(
				doTestCount(".calendar .event .symbol .fa-fw")
			).resolves.toBe(2);
			await expect(
				doTestCount(".calendar .event .symbol .fa-calendar-check")
			).resolves.toBe(1);
			await expect(
				doTestCount(".calendar .event .symbol .fa-google")
			).resolves.toBe(1);
		});
	});
	...
	describe("count events broadcast", () => {
		it("get 12 with maxentries set to 1", async () => {
			await helpers.startApplication(
				...
				"01 Jan 2024 12:30:00 GMT-076:00",
				[],
				"America/Chicago"
			);
			await expect(
				doTestTableContent(".testNotification", ".elementCount", "12", first)
			).resolves.toBe(true);
		});
	});
});
