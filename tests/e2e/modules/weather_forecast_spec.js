	...
	describe("Default configuration", () => {
		...
		for (const [index, day] of days.entries()) {
			it(`should render day ${day}`, async () => {
				await expect(
					weatherFunc.getText(
						`.weather table.small tr:nth-child(${index + 1}) td:nth-child(1)`,
						day
					)
				).resolves.toBe(true);
			});
		...
		for (const [index, icon] of icons.entries()) {
			it(`should render icon ${icon}`, async () => {
				const elem = await helpers.waitForElement(
					`.weather table.small tr:nth-child(${index + 1}) td:nth-child(2) span.wi-${icon}`
				);
		...
		for (const [index, temp] of maxTemps.entries()) {
			it(`should render max temperature ${temp}`, async () => {
				await expect(
					weatherFunc.getText(
						`.weather table.small tr:nth-child(${index + 1}) td:nth-child(3)`,
						temp
					)
				).resolves.toBe(true);
			});
		...
		for (const [index, temp] of minTemps.entries()) {
			it(`should render min temperature ${temp}`, async () => {
				await expect(
					weatherFunc.getText(
						`.weather table.small tr:nth-child(${index + 1}) td:nth-child(4)`,
						temp
					)
				).resolves.toBe(true);
			});
		...
		for (const [index, opacity] of opacities.entries()) {
			it(`should render fading of rows with opacity=${opacity}`, async () => {
				const elem = await helpers.waitForElement(
					`.weather table.small tr:nth-child(${index + 1})`
				);
				expect(elem).not.toBeNull();
		...
		for (const [index, precipitation] of precipitations.entries()) {
			if (precipitation) {
				it(`should render precipitation amount ${precipitation}`, async () => {
					await expect(
						weatherFunc.getText(
							`.weather table tr:nth-child(${index + 1}) td.precipitation-amount`,
							precipitation
						)
					).resolves.toBe(true);
				});
			...
			for (const [index, precipitation] of precipitations.entries()) {
				if (precipitation) {
					it(`should render precipitation amount ${precipitation}`, async () => {
						await expect(
							weatherFunc.getText(
								`.weather table.small tr:nth-child(${index + 1}) td.precipitation-amount`,
								precipitation
							)
						).resolves.toBe(true);
					});
