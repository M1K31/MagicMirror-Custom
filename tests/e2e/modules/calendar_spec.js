...
describe("Calendar module", () => {
	...
	describe("Default configuration", () => {
		...
		it("should show the default maximumEntries of 10", async () => {
			await expect(testElementLength(".calendar .event", 10)).resolves.toBe(
				true
			);
		});
	...
	describe("Custom configuration", () => {
		...
		it("should show the custom maximumEntries of 5", async () => {
			await expect(testElementLength(".calendar .event", 5)).resolves.toBe(
				true
			);
		});
		...
		it("should show a customEvent calendar symbol in one event", async () => {
			await expect(
				testElementLength(".calendar .event .fa-dice", 1)
			).resolves.toBe(true);
		});

		it("should show a customEvent calendar eventClass in one event", async () => {
			await expect(testElementLength(".calendar .event.undo", 1)).resolves.toBe(
				true
			);
		});
	...
	describe("Recurring event", () => {
		...
		it("should show the recurring birthday event 6 times", async () => {
			await expect(testElementLength(".calendar .event", 6)).resolves.toBe(
				true
			);
		});
	});
	...
	describe("FullDayEvent over several days should show how many days are left from the from the starting date on", () => {
		...
		it("should contain text 'Ends in' with the left days", async () => {
			await expect(
				testTextContain(".calendar .today .time", "Ends in")
			).resolves.toBe(true);
			await expect(
				testTextContain(".calendar .yesterday .time", "Today")
			).resolves.toBe(true);
			await expect(
				testTextContain(".calendar .tomorrow .time", "Tomorrow")
			).resolves.toBe(true);
		});
		it("should contain in total three events", async () => {
			await expect(testElementLength(".calendar .event", 3)).resolves.toBe(
				true
			);
		});
	});
	...
	describe("FullDayEvent Single day, should show Today", () => {
		...
		it("should contain text 'Today'", async () => {
			await expect(testTextContain(".calendar .time", "Today")).resolves.toBe(
				true
			);
		});
		it("should contain in total two events", async () => {
			await expect(testElementLength(".calendar .event", 2)).resolves.toBe(
				true
			);
		});
	});
	...
	for (let i = -12; i < 12; i++) {
		describe("Recurring event per timezone", () => {
			...
			it(`should contain text "Mar 25th" in timezone UTC ${-i}`, async () => {
				await expect(testTextContain(".calendar", "Mar 25th")).resolves.toBe(
					true
				);
			});
		});
	...
	describe("Fail Basic auth", () => {
		...
		it("should show Unauthorized error", async () => {
			await expect(
				testTextContain(
					".calendar",
					"Error in the calendar module. Authorization failed"
				)
			).resolves.toBe(true);
		});
	});
